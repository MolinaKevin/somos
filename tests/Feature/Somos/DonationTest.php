...
it('cannot create a package of donations if total amount exceeds donated points', function () {
    ...
    $donationService = new DonationService(); // Reemplaza esto con tu método preferido para obtener una instancia de tu servicio.
...
it('cannot create a package of donations if total amount leaves more than 1 donated point', function () {
    $commerce = Commerce::factory()->create(['gived_points' => 100]);

    $nros = Nro::factory()->count(3)->create();

    $donationData = [
        ['nro' => $nros[0], 'amount' => 32.50],
        ['nro' => $nros[1], 'amount' => 33.50],
        ['nro' => $nros[2], 'amount' => 33.00],
    ];
